...
* **Object Path:** `/org/mpris/MediaPlayer2`
* **Bus Type:** Hybrid Bus. The client tries to bind to the **Session Bus** if the `DBUS_SESSION_BUS_ADDRESS` environment variable is defined. If absent (e.g., on a headless Raspberry Pi boot environment), it **automatically falls back to the System Bus**.
...
* **`Volume`** (`Double`) — Local soundcard hardware/software volume level `[0.0..1.0]`. Writing to this updates the local mixer gain and notifies the server.
...
`playerctl` automatically detects MPRIS2 interfaces.

```bash
# List all active SnapDog server zones and client players
playerctl --list-all | grep snapdog

# Toggle Play/Pause on Server Zone 1
playerctl -p snapdog.zone1 play-pause

# Skip to the next track on Client Player 1
playerctl -p snapdog_client next

# Set volume of Client Player to 80% (0.8)
playerctl -p snapdog_client volume 0.8
```

### Control using `busctl`

For low-level scripting or diagnostics:

```bash
# Query Server Zone 1 playback status
...
# Query Client Player volume status (System bus fallback)
