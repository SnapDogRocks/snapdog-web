...
|---|---|---:|---|
| `type` | string | yes | Always `playback_error`. |
| `zone` | number | yes | 1-based zone index that encountered the failure. |
| `message` | string | yes | Primary user-facing error message. |
| `details` | string \| null | no | Optional technical diagnostic detail. The field may be omitted or `null`. |
| `recoverable` | boolean | yes | Whether the server considers the error recoverable without explicit user action. |
